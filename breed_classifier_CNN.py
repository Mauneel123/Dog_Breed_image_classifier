...
g = nn.Linear(out_size, 1000, bias=True).to(device) # changed 1
...
params = list(g.parameters())# + list(Gamma.parameters())#+ <NEED TO CHANGE HERE> list(Gamma.parameters())
...
def train_model(Gamma, g, criterion, optimizer, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        ## (CHANGE HERE)
        #Gamma.<MAKE SURE GAMMA IS IN TEST MODE>() # Set Gamma to test mode
        Gamma.eval()
        ...
        #while <CRITERIA LOOP FOR CONSUMER>:
        while True:

            batchIdx, inputs, labels = my_queue.get()

            #<CRITERIA FOR CONSUMER TO STOP While LOOP>
            if inputs == None:
                break                    
            ...
            if mixed_precision:
                ...
                with autocast():
                    ...
                    #outputs = g(Gamma(inputs)<CHANGE HERE>) 
            ...
            #<ADD HERE: Missing step in mixed precision optimization>
            if mixed_precision:
